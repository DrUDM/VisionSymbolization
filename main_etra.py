...
import processing.ETRA.symbolization as sy
from processing.ETRA.clustering import Clustering
...
def symbolization(config=None):
    ...
    sy.process(config, path, feature_records)
...
def clustering(config=None):
    ...
    path = 'output/results/ETRA/symbolization/'
    symb_results = [f for f in os.listdir(path) if f[-4:] == '.pkl']
   
    cl = Clustering(config, path, symb_results)
    cl.process()
