...
class BlockAnalysis():
    ...
    def __init__(self, 
        ...
        self.process_oculomotor=True 
        self.process_scanpath=True
        self.process_aoi=True 
    ...
    def process(self):
        ...
        for record in sorted(self.records):   
            ...
            if (subject in config['data']['subject_set'] and label in config['data']['label_set']):   
              #if label=='kdrive_baseline_3' :
                print('\nAnalyzing file {rec_}'.format(rec_=record))   
                ## Load corresponding DataFrame
                df = pd.read_csv(self.path+record) 
                
                if len(df)>config['general']['record_min_length']:
    ...
    def process_oculomotor_features_(self, 
        ...
        filename='output/ADABase/features/{r_}_oculomotor.csv'.format(r_=record.split('.')[0])
    ...
    def process_scanpath_features_(self,  
        ...
        filename='output/ADABase/features/{r_}_scanpath.csv'.format(r_=record.split('.')[0])
        result_df.to_csv(filename, index=False)   
    ...
    def process_aoi_features_(self, 
        ...
        filename='output/ADABase/features/{r_}_AoI.csv'.format(r_=record.split('.')[0])
        result_df.to_csv(filename, index=False)  
